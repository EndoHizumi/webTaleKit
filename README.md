...
8. (サーバー側) {リソース名}.jsをレスポンスとして返す。
9. （エンジン）帰ってきたjsファイルをダイナミックインポートからの実行して、UI画面の描画･ゲームの進行を行う。
10. routeタグがあった場合は、6からフローを実行する。
11. ファイルの末尾に到達した場合、エンジンの実行を終了する。
...
- 画面の表示・移動 -次のシーンに移動（routeタグは、ダイナミックインポートに置換される）
  - 対応するWTS: `<route to="">`
- 選択肢の表示・定義
