export function outputLog(msg='', level = 'log', option = undefined) {
    const error = new Error();
    const stack = error.stack?.split('\n') || [];
    // スタックトレースの3番目の要素が呼び出し元の情報
    const callerInfo = stack[2].trim();
    // 呼び出し元の情報から行数とメソッド名を抽出
    const [, callerMethod, callerLine] = callerInfo.match(/at (.+) \((.+)\)/);
    console[level](level.toUpperCase(), msg, callerMethod, option||undefined, callerLine);
  }
