...
import { EngineConfig, SceneConfig } from '../types/config'
import {
  ScenarioObject,
  TextHandlerScenario,
  WaitHandlerScenario,
...
import { DisplayedImage, UsedSound, SceneFile } from '../types/resource'
import { CommandList, CenterPoint, ChoiceResult } from '../types/api'

export class Core {
  private bgm: SoundObject | null = null;
  private isAuto: boolean = false;
  private isNext: boolean = false;
  private isSkip: boolean = false;
  private onNextHandler: (() => void) | null = null;
  private sceneFile: SceneFile = {} as SceneFile;
  private sceneConfig: SceneConfig = {} as SceneConfig;
  private commandList: CommandList;
  private gameContainer: HTMLElement;
  private drawer: Drawer;
  private scenarioManager: ScenarioManager;
  private resourceManager: ResourceManager;
  private displayedImages: { [key: string]: DisplayedImage } = {};
  private usedSounds: { [key: string]: UsedSound } = {};
  private tempImages: { [key: string]: DisplayedImage } = {};
  ...
  async start(initScene?: string): Promise<void> {
    ...
    // titleタグの内容を書き換える
    document.title = (engineConfig as EngineConfig).title;
    // sceneファイルを読み込む
    ...
    document.querySelector('#gameContainer')!.addEventListener('keydown', (e: Event) => {
      if ((e as KeyboardEvent).key === 'Enter') {
        this.onNextHandler?.();
      } else if ((e as KeyboardEvent).key === 'Control') {
  ...
  async loadScreen(sceneConfig: SceneConfig): Promise<void> {
    ...
    // ゲーム進行用に必要な情報をセットする
    this.drawer.setScreen(this.gameContainer, (engineConfig as EngineConfig).resolution);
    // シナリオの進行状況を保存
  ...
  async runScenario(): Promise<void> {
    ...
    const commandFunction = this.commandList[commandType as keyof CommandList];
  ...
  async textHandler(scenarioObject: TextHandlerScenario | string): Promise<void> {
  ...
  async waitHandler(line: WaitHandlerScenario): Promise<void> {
  ...
  async sayHandler(line: SayHandlerScenario): Promise<void> {
    outputLog('call', 'debug', line);
    // say(name:string, pattern: string, voice: {playの引数},  ...text)
    if (line.voice) await this.soundHandler({ src: line.voice, play: undefined });
    await this.textHandler({ content: line.content, name: line.name, speed: line.speed || 25 });
    this.scenarioManager.setHistory(line as any);
  }

  async choiceHandler(line: ChoiceHandlerScenario): Promise<void> {
    outputLog('call', 'debug', line);
    document.querySelector('#interactiveView')!.style.visibility = 'visible';
    if (line.prompt) await this.textHandler(line.prompt);
  ...
  jumpHandler(line: JumpHandlerScenario): void {
    ...
    if (line.index! < this.scenarioManager.getIndex()) {
      ...
      // sub=falseの行だけを取得
      const subFalseScenario = scenarioList.filter((line: ScenarioObject) => !line.sub);
  ...
  async showHandler(line: ShowHandlerScenario): Promise<void> {
    outputLog('line', 'debug', line);
    // ムスタッシュ構文があるときは、変数の展開
    Object.keys(line).forEach((item) => {
      (line as any)[item] = this.expandVariable((line as any)[item]);
    });
    // 表示する画像の情報を管理オブジェクトに追加
    const modeList: { [key: string]: string } = { bg: 'background', cutin: '', chara: '', cg: 'background', effect: 'effect' };
    const key = Object.keys(modeList).includes(line.mode!) ? modeList[line.mode!] : line.name || line.src!.split('/').pop()!;
    const baseLine = (engineConfig as EngineConfig).resolution.height / 2;
    const centerPoint: { [key: string]: CenterPoint } = {
      left: { x: (engineConfig as EngineConfig).resolution.width * 0.25, y: baseLine },
      center: { x: (engineConfig as EngineConfig).resolution.width * 0.5, y: baseLine },
      right: { x: (engineConfig as EngineConfig).resolution.width * 0.75, y: baseLine },
    };
    ...
    if (line.mode === 'cg') {
      ...
      this.displayedImages = { background: this.displayedImages.background };
      size = { width: (engineConfig as EngineConfig).resolution.width, height: (engineConfig as EngineConfig).resolution.height };
    }
    ...
    if (line.pos) {
      ...
      const baseLines: { [key: string]: number } = {
        top: 0 + size.height,
        middle: (engineConfig as EngineConfig).resolution.height / 2,
        bottom: (engineConfig as EngineConfig).resolution.height - size.height,
      };
  ...
  async hideHandler(line: HideHandlerScenario): Promise<void> {
    outputLog('call', 'debug', line);
  ...
  async moveToHandler(line: MoveToHandlerScenario): Promise<void> {
  ...
  async getImageObject(line: ShowHandlerScenario): Promise<ImageObject> {
    outputLog('call', 'debug', line);
  ...
  async soundHandler(line: SoundHandlerScenario): Promise<void> {
    outputLog('call', 'debug', line);
    let soundObject: SoundObject | null = null;
    if (line.mode === 'bgm') {
  ...
  async getSoundObject(line: SoundHandlerScenario): Promise<SoundObject> {
    outputLog('call', 'debug', line);
    const name = line.name || line.src!.split('/').pop()!;
  ...
  async ifHandler(line: IfHandlerScenario): Promise<void> {
    outputLog('call', 'debug', line);
  ...
  async routeHandler(line: RouteHandlerScenario): Promise<void> {
    outputLog('call', 'debug', line);
    if (this.bgm?.isPlaying) {
      this.bgm.stop();
      this.bgm = null;
  ...
  async httpHandler(line: ScenarioObject): Promise<ScenarioObject> {
    ...
    if (progressText) {
      await this.textHandler({ content: [progressText.content][0] });
    }
  ...
  getAPIForScript(): any {
    return {
      ...
      scenario: {
        ...
        setProgress: (progress: any) => (this.scenarioManager.progress = progress),
        ...
        setHistory: (history: any) => this.scenarioManager.setHistory(history),
        setScenario: (scenario: ScenarioObject[]) => this.scenarioManager.setScenario(scenario),
