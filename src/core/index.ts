...
let engineConfig: any = require('../../engineConfig.json')

interface LoadScreenOptions {
  isDialog?: boolean
...
export class Core {
  private bgm: SoundObject | null
  private isAuto: boolean
  private isNext: boolean
  private isSkip: boolean
  private onNextHandler: (() => void) | null
  private sceneFile: any
  private sceneConfig: any
  private commandList: Record<string, (line: any) => Promise<void> | void>
  private gameContainer: HTMLElement
  private drawer: Drawer
  private scenarioManager: ScenarioManager
  private resourceManager: ResourceManager
  private displayedImages: Record<string, any>
  private usedSounds: Record<string, any>
  private store: Store
  private tempImages: Record<string, any>
  ...
  async start(initScene: string): Promise<void> {
    try {
      ...
      // 入力イベントを設定する
      document.querySelector('#gameContainer')!.addEventListener('keydown', (e: Event) => {
      ...
      document.querySelector('#gameContainer')!.addEventListener('keyup', (e: Event) => {
        const ke = e as KeyboardEvent
        if (ke.key === 'Control') {
          this.drawer.isSkip = true
          this.isNext = false
  ...
  // ファイルの存在確認を行う関数
  async checkResourceExists(url: string): Promise<boolean> {
    try {
  ...
  async loadScreen(sceneConfig: any, options: LoadScreenOptions = {}): Promise<void> {
    const {
    ...
    // 画面名を設定する。
    ;(this.scenarioManager as any).progress.currentScene = sceneConfig.name
    ...
    let mainDiv: HTMLElement | null = isDialog ? doc.getElementById('dialogContainer') : doc.getElementById('main')
  ...
  async runScenario(): Promise<void> {
    ...
    const commandType: string = scenarioObject.type || 'text'
  ...
  async textHandler(scenarioObject: any): Promise<void> {
  ...
  async waitHandler(line: any): Promise<void> {
  ...
  async sayHandler(line: any): Promise<void> {
  ...
  async choiceHandler(line: any): Promise<void> {
    ;(document.querySelector('#interactiveView') as HTMLElement).style.visibility = 'visible'
    ...
    if (selectHandler !== undefined) {
      this.scenarioManager.addScenario(selectHandler, 0)
    }
    this.scenarioManager.setHistory({ line, selectId } as any)
    ;(document.querySelector('#interactiveView') as HTMLElement).style.visibility = 'hidden'
    this.isNext = false
  }

  jumpHandler(line: any): void {
  ...
  async showHandler(line: any): Promise<void> {
    // ムスタッシュ構文があるときは、変数の展開
  ...
  async hideHandler(line: any): Promise<void> {
  ...
  async moveToHandler(line: any): Promise<void> {
    const key = line.name
    await this.drawer.moveTo(key, this.displayedImages, { x: line.x, y: line.y }, line.duration || 1)
  }

  async getImageObject(line: any): Promise<ImageObject> {
    ...
    if (Object.prototype.hasOwnProperty.call(this.displayedImages, name)) {
  ...
  async soundHandler(line: any): Promise<void> {
    ...
    if (line.mode === 'bgm') {
      ...
      if ('play' in line) {
        'loop' in line ? soundObject.play(true) : soundObject.play(false)
      }
    }
    ...
    const key = line.name || (line.src ? line.src.split('/').pop() : 'bgm')
    this.usedSounds[key] = {
      audio: soundObject,
    }
  }

  async getSoundObject(line: any): Promise<SoundObject> {
    const name = line.name || (line.src ? line.src.split('/').pop() : 'bgm')
    ...
    // 既にインスタンスがある場合は、それを使う
    if (Object.prototype.hasOwnProperty.call(this.usedSounds, name)) {
      const targetResource = this.usedSounds[name]
      const soundObject: SoundObject = targetResource ? targetResource.audio : new SoundObject()
  ...
  async ifHandler(line: any): Promise<void> {
    ...
    this.scenarioManager.addScenario(appendScenario, 0)
  }

  async routeHandler(line: any): Promise<void> {
    if (this.bgm && this.bgm.playing) {
      // BGMを停止する
      this.soundHandler({
        mode: 'bgm',
  ...
  callHandler(line: any): void {
    this.executeCode(line.method)
  }

  async httpHandler(line: any): Promise<any> {
    if (!(line.get || line.post || line.put || line.delete)) {
      return line
    }
    // progress属性を処理する
    // prettier-ignore
    const progressText = line.content.filter((content: any) => content.type === 'progress')[0]
    if (progressText) {
    ...
    const headers = line.content
      ...
      .content.reduce(
        (acc: Record<string, any>, header: any) => ({
          ...acc,
  ...
  async dialogHandler(scenarioObject: any): Promise<any> {
    let result: any = null
    ...
    scenarioObject.content.forEach((content: any) => {
      if (content.type === 'prompt') {
        ...
        if (promptElement) {
          promptElement.innerHTML = prompt.content.map((text: any) => this.expandVariable(text)).join('\n')
        }
        ...
        actions.forEach((action: any) => {
          ...
          // テンプレートのボタン取得
          let button = buttonContainer!.querySelector(`#dialog-button-${action.id}`) as HTMLButtonElement | null
          ...
          button.addEventListener('click', () => {
            // 選択されたアクションを処理
            this.scenarioManager.addScenario(action.content, 0)
  ...
  getAPIForScript(): any {
    return {
      ...
      scenario: {
        ...
        getProgress: () => (this.scenarioManager as any).progress,
        setProgress: (progress: any) => ((this.scenarioManager as any).progress = progress),
        ...
        setHistory: (history: string) => this.scenarioManager.setHistory(history),
        setScenario: (scenario: any) => this.scenarioManager.setScenario(scenario),
        getScenario: () => this.scenarioManager.getScenario(),
        getSceneName: () => (this.scenarioManager as any).progress.currentScene,
        setScreenName: (name: string) => (this.sceneConfig.name = name),
      },
      images: {
        get: this.getImageObject.bind(this),
        getAll: () => this.displayedImages,
        set: (name: string, image: any) => (this.displayedImages[name] = image),
        delete: (name: string) => delete this.displayedImages[name],
      },
      sounds: {
        get: (name: string) => this.usedSounds[name],
        getAll: () => this.usedSounds,
        set: (name: string, sound: any) => (this.usedSounds[name] = sound),
      ...
      save: {
        ...
        setSaveData: (data: any) => this.setSaveData(data),
  ...
  async saveHandler(line: any): Promise<void> {
    ...
    const saveData = {
      ...
      scenarioManager: {
        progress: JSON.parse(JSON.stringify((this.scenarioManager as any).progress)),
      ...
      displayedImages: Object.keys(this.displayedImages).reduce((acc: Record<string, any>, key) => {
        if (key !== 'background') {
          acc[key] = {
      ...
      usedSounds: Object.keys(this.usedSounds).reduce((acc: Record<string, any>, key) => {
        acc[key] = {
  ...
  async loadHandler(line: any): Promise<void> {
    const slot = line.slot || 'auto'

    const saveDataRaw = (this.store as any).get ? (this.store as any).get(`save_${slot}`) : this.store[`save_${slot}`]
    if (!saveDataRaw) {
    ...
    ;(this.scenarioManager as any).progress = { ...(this.scenarioManager as any).progress, ...saveData.scenarioManager.progress }
    ...
    for (const [key, imageData] of Object.entries(saveData.displayedImages) as [string, any][]) {
      if (imageData.src) {
