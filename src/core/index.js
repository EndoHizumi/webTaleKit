...
export class Core {
  ...
  async showHandler(line) {
    ...
    const layerOrder = Number(line['z-index'])
    this.displayedImages[key]['z-index'] = Number.isFinite(layerOrder) ? layerOrder : 0
      ...
      displayedImages: Object.keys(this.displayedImages).reduce((acc, key) => {
        if (key !== 'background') {
          acc[key] = {
            ...
            'z-index': this.displayedImages[key]['z-index'],
          }
        }
        return acc
      }, {}),
    ...
    for (const [key, imageData] of Object.entries(saveData.displayedImages)) {
      if (imageData.src) {
        ...
        this.displayedImages[key] = {
          ...
          'z-index': Number.isFinite(Number(imageData['z-index'])) ? Number(imageData['z-index']) : 0,
        }
      }
    }
