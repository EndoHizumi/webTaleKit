...
```html
<text speed="50" wait="true" clear="true">表示するテキスト</text>
```
...
| 属性 | 型 | 必須 | デフォルト | 説明 |
|------|-----|------|-----------|------|
| speed | number | × | 25 | 1文字ごとの表示間隔（ミリ秒）。小さいほど速い |
...
`speed` は1文字ごとの表示間隔をミリ秒で指定します（デフォルトは25ミリ秒）。

### 速く表示

```html
<text speed="10">素早く表示されるテキスト</text>
```

### ゆっくり表示

```html
<text speed="100">ゆっくり表示されるテキスト</text>
```

### 最速で表示

```html
<text speed="0">最も速く表示されるテキスト</text>
```
...
- `speed` は1文字ごとの表示間隔をミリ秒で指定します（50 = 1文字ごとに50ミリ秒待つ）
